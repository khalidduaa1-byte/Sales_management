...
- **`profiles`** — extends `auth.users` with `role` (`manager`|`ba`), `team`, `store`, `start_date` (first active day; the Missing Sales calendar hides a BA in months before it. Backfilled from earliest activity; new signups stamped at registration via `handle_new_user`; pulled earlier when legacy rows are linked), `end_date` (last active day for a BA who **left the company**; `null` = still here — see "Departed BAs" below)
...
- **Departed BAs are marked with `profiles.end_date`, never deleted.** A BA who leaves the company gets `end_date` = their last working day (`web_app/sql/add_profiles_end_date.sql`; `null` = still here). The dashboard then drops them from `teamHeadcount` (so per_ba targets shrink to the BAs actually there — past months are safe because team size falls back to `Math.max(headcount, BAs who logged sales that month)`), hides them from the Missing Sales calendar in months after they left, greys out the days after their last day in the month they left, and labels them `(left)` in the BA pickers. **Their `sales_entries` rows stay** — that revenue is booked in the months they worked, and deleting it would silently change every past month's team total and attainment. Selecting a past month still shows them. Symmetric with `start_date`; marking someone else departed later is one row added to the values list in that SQL file.
...
- **Animation calendar (data-driven, replaced the hardcoded HPP block).** Animation pop-up spots now live in the `animation_events` Supabase table (see `web_app/sql/add_animation_calendar.sql`). `ba.html` `loadActiveAnimations()` fetches events for the BA's team where today is within `start_date … end_date + entry_buffer_days`; when any are active, `renderAnimationCard()` shows a **dedicated "🎯 Animation Sale" card** (separate from the regular Log-a-Sale form) so BAs record animation sales distinctly from regular-shop sales on the same day. Both forms write via the shared `upsertSale()` helper to `sales_entries` (different `store`, so they never collide). Animation spots are deliberately NOT in the regular shop dropdown. No code change to add a future animation — just an `insert`. Labels follow the `'Terminal N — <code> (animation)'` form (e.g. `'Terminal 3 — 702A (animation)'`); the dashboard strips the `(animation)` tag for display via `displayAnimName()` but the stored `store` keeps it for detection. The June 2026 pilot was relabelled from `HPP` to `702A` (`sql/rename_june_animation_to_702a.sql`). Managers can set each event's `target_pcs_per_day` inline from the Animations overview. `manager.html` `normalizeStore` maps `/animation|hpp/` to its own sub-shop (kept out of regular shop totals), and `isHppEntry` (`/animation|hpp/`) drives the **Animations** tab.
- **Historical animation actuals are reference-only.** Past animations (e.g. Jan–Feb 2026 "The One" @ T3 702A) are ALREADY inside the monthly Excel totals, so their dailies live in the separate `animation_daily` table and feed ONLY the Animations tab (synthetic rows, `_historical` flag, no BA). They are **never** inserted into `sales_entries` — doing so double-counts (Invariant #3).
